...
class ClientPresentation:
    ...
    def create_executive_summary(self, save_path=None):
        ...
        fig.suptitle('PERFIL DOS GRUPOS DE PACIENTES OFTALMOLÓGICOS\nResumo Executivo', 
                     fontsize=18, fontweight='bold', y=0.95)
        
        # 1. Distribuição dos pacientes por grupo
        colors = ['#FF6B6B', '#4ECDC4', '#45B7D1']
        wedges, texts, autotexts = ax1.pie(self.cluster_counts.values, 
        ...
        ax1.set_title('Distribuição de Pacientes por Grupo', fontsize=14, fontweight='bold', pad=20)
        
        # Adicionar contagem absoluta
        for i, (count, autotext) in enumerate(zip(self.cluster_counts.values, autotexts)):
        ...
        for i, (cluster_data, color) in enumerate(zip(normalized_data, colors)):
            ...
            ax2.plot(angles, values, 'o-', linewidth=2, label=f'Grupo {i+1}', color=color)
            ax2.fill(angles, values, alpha=0.25, color=color)
        
        ax2.set_xticks(angles[:-1])
        ax2.set_xticklabels(categories)
        ax2.set_ylim(0, 1)
        ax2.set_title('Perfil Comparativo dos Grupos\n(Valores Normalizados)', 
                      fontsize=14, fontweight='bold', pad=20)
        ax2.legend(loc='upper right', bbox_to_anchor=(1.3, 1.0))
        
        # 3. Boxplot das principais características
        features_to_plot = ['AL', 'K1']
        data_melted = pd.melt(self.data[features_to_plot + ['Cluster']], 
                             id_vars=['Cluster'], 
                             var_name='Característica', 
                             value_name='Valor')
        
        sns.boxplot(data=data_melted, x='Característica', y='Valor', hue='Cluster', 
                   palette=colors, ax=ax3)
        ax3.set_title('Distribuição das Principais Características', fontsize=14, fontweight='bold')
        ...
        table = ax4.table(cellText=summary_data,
                         colLabels=['Grupo', 'Pacientes', 'Comprimento Axial\n(AL mm)', 
                                   'Curvatura K1\n(Dioptrias)', 'Curvatura K2\n(Dioptrias)'],
                         cellLoc='center',
                         loc='center',
        ...
        ax4.set_title('Resumo Estatístico por Grupo', fontsize=14, fontweight='bold', pad=20)
    ...
    def create_detailed_profiles(self, save_path=None):
        ...
        fig.suptitle('PERFIS DETALHADOS DOS GRUPOS DE PACIENTES', 
                     fontsize=18, fontweight='bold', y=0.98)
        
        colors = ['#FF6B6B', '#4ECDC4', '#45B7D1']
        group_names = ['Grupo 1: Olhos Curtos', 'Grupo 2: Olhos Longos', 'Grupo 3: Olhos Normais']
        ...
        for cluster in range(3):
            ...
            means = [self.cluster_stats.loc[cluster, (feature, 'mean')] for feature in features]
            stds = [self.cluster_stats.loc[cluster, (feature, 'std')] for feature in features]
            
            x_pos = np.arange(len(features))
            bars = ax1.bar(x_pos, means, yerr=stds, capsize=5, 
                          color=colors[cluster], alpha=0.7, edgecolor='black')
            
            ax1.set_xlabel('Características Oculares')
            ax1.set_ylabel('Valores')
            ax1.set_title(f'{group_names[cluster]}\n({self.cluster_counts[cluster]} pacientes)', 
                         fontweight='bold')
            ...
            # Scatter plot 2D
            ax2 = axes[cluster, 1]
            scatter = ax2.scatter(cluster_data['AL'], cluster_data['K1'], 
                                 c=colors[cluster], alpha=0.6, s=30)
            ...
            p = np.poly1d(z)
            ax2.plot(cluster_data['AL'], p(cluster_data['AL']), "--", color='red', alpha=0.8)
    ...
    def create_clinical_interpretation(self, save_path=None):
        ...
        """
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('INTERPRETAÇÃO CLÍNICA DOS GRUPOS IDENTIFICADOS', 
                     fontsize=18, fontweight='bold', y=0.95)
        ...
        for cluster in range(3):
            cluster_data = self.data[self.data['Cluster'] == cluster]
            ax1.scatter(cluster_data['AL'], cluster_data['K1'], 
                       c=colors[cluster], label=f'Grupo {cluster+1}', alpha=0.6, s=30)
        
        ax1.set_xlabel('Comprimento Axial (mm)')
        ax1.set_ylabel('Curvatura Corneana K1 (Dioptrias)')
        ax1.set_title('Relação Comprimento-Curvatura\n(Diagnóstico de Ametropia)', fontweight='bold')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        ...
        for cluster in range(3):
            cluster_data = self.data[self.data['Cluster'] == cluster]
            ax2.hist(cluster_data['ACD'], bins=20, alpha=0.7, color=colors[cluster], 
                    label=f'Grupo {cluster+1}', density=True)
        
        ax2.set_xlabel('Profundidade da Câmara Anterior (mm)')
        ax2.set_ylabel('Densidade de Probabilidade')
        ax2.set_title('Distribuição da Profundidade\nda Câmara Anterior', fontweight='bold')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        for cluster in range(3):
            ...
            ax3.scatter(cluster_data['K1'], cluster_data['K2'], 
                       c=astigmatism, s=30, alpha=0.7, cmap='YlOrRd')
        ...
        ax3.plot([min_k, max_k], [min_k, max_k], '--', color='black', alpha=0.5, 
        ...
        clinical_interpretation = [
            ['Grupo 1\n(Olhos Curtos)', 'AL < 23mm\nK > 45D', 'Hipermetropia\nRisco: Glaucoma', 'Lentes divergentes\nMonitoramento IOP'],
            ['Grupo 2\n(Olhos Longos)', 'AL > 24mm\nK < 42D', 'Miopia\nRisco: Degeneração', 'Lentes convergentes\nExame de retina'],
            ['Grupo 3\n(Olhos Normais)', '23-24mm\n42-45D', 'Emetropia\nBaixo risco', 'Acompanhamento\npreventivo']
        ]
        
        table = ax4.table(cellText=clinical_interpretation,
        ...
        ax4.set_title('Interpretação e Recomendações Clínicas', fontsize=14, fontweight='bold', pad=20)
    ...
    def generate_report(self):
        ...
        print(f"• Características analisadas: Comprimento Axial, Profundidade da Câmara, Diâmetro Corneano, Curvaturas")
        ...
        for i, (cluster, count) in enumerate(self.cluster_counts.items()):
            ...
            print(f"  - Comprimento Axial: {self.cluster_stats.loc[cluster, ('AL', 'mean')]:.2f} ± {self.cluster_stats.loc[cluster, ('AL', 'std')]:.2f} mm")
            print(f"  - Curvatura K1: {self.cluster_stats.loc[cluster, ('K1', 'mean')]:.2f} ± {self.cluster_stats.loc[cluster, ('K1', 'std')]:.2f} D")
            print(f"  - Curvatura K2: {self.cluster_stats.loc[cluster, ('K2', 'mean')]:.2f} ± {self.cluster_stats.loc[cluster, ('K2', 'std')]:.2f} D")
        
        print(f"\n💡 INSIGHTS PARA O NEGÓCIO:")
        print(f"• Grupo mais numeroso: Grupo 3 ({self.cluster_counts[2]:,} pacientes - {(self.cluster_counts[2]/self.cluster_counts.sum())*100:.1f}%)")
        print(f"• Oportunidade de especialização em correção miópica (Grupo 2)")
        print(f"• Necessidade de monitoramento especializado para Grupo 1 (risco de glaucoma)")
